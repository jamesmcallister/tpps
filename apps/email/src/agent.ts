...
function extractReplyOnlyText(text: string): string {
  ...
  const inlineReplyCut = withoutQuotedLines.split(/\n(?=On [^\n]{0,500}? wrote:\s*)/i)[0].trim();
...
export function extractBestEmailText(body: string | null | undefined): string {
  ...
  const plainText = /<[a-z][\s\S]*>/i.test(body) ? htmlToReplyText(body) : body;
...
export function buildKnowledgeContext(knowledge: CompanyKnowledge): string {
  ...
  if (knowledge.contactDetails) {
    const contactLines = [
      ...
      knowledge.contactDetails.serviceArea
        ? `Service area: ${knowledge.contactDetails.serviceArea}`
        : null,
...
export function classifyIncomingEmail(params: {
  ...
  const scoredCategories: Array<{
    ...
    {
      category: "supplier_printer_collaboration",
      matches: includesAny(haystack, SUPPLIER_KEYWORDS),
    },
    { category: "sales_new_enquiry", matches: includesAny(haystack, SALES_KEYWORDS) },
  ];

  const topMatch = scoredCategories.sort(
...
function extractEstimatedArea(bodyText: string): number | null {
  const match = bodyText.match(
    /\b(\d{1,5})\s*(sqm|sq m|m2|m²|square metres|square meters|meters|metres|sq ft)\b/i,
  );
  if (!match) {
    return null;
  }
...
function extractInterestsOrProblems(bodyText: string): string[] {
  ...
  const relevant = sentences.filter((sentence) =>
    /(garden|landscaping|patio|driveway|fence|fencing|groundwork|tree|site|quote|price|pricing|estimate)/i.test(
      sentence,
    ),
  );
...
function summariseLatestThread(params: {
  ...
  return `${params.classification.category}: ${firstSentence ?? "No clear summary available."}`.slice(
    0,
    220,
  );
}
...
export function updateContactMemory(params: {
  ...
  const propertyOrCompanyName = extractPropertyOrCompanyName(
    params.bodyText,
    params.existing.senderEmail,
  );
  const propertyType = extractPropertyType(params.bodyText);
  const estimatedArea = extractEstimatedArea(params.bodyText);
  const interestsOrProblems = Array.from(
    new Set([
...
function isQuoteStyleEnquiry(subject: string, text: string): boolean {
  return /\bquote\b|\bprice\b|\bpricing\b|\bcost\b|\bhow much\b|\bestimate\b/i.test(
    `${subject}\n${text}`,
  );
}
...
function isQuoteFollowUpDetailQuestion(params: {
  ...
  const hasDetailSignal =
    /\b(resin|sparkles|colour|color|finish|edging|border|block paving|tarmac|gravel|pattern|premium|budget|photo|picture|image|slope|access|surface|concrete|cracked|next step)\b/i.test(
      haystack,
    );
  const hasExistingThreadContext = params.threadContext !== "No earlier messages in this thread.";

  return (
    hasReplyQuoteSubject ||
...
function buildQuoteFollowUpReplyContext(params: {
  ...
  const guidanceLines = [
    ...
    detectedTopics.length > 0
      ...
      : "Detected topics: general finish or material details.",
  ...
  if (params.knowledge.followUpGuidance?.principles?.length) {
    guidanceLines.push(
      `Follow-up principles: ${params.knowledge.followUpGuidance.principles.join("; ")}`,
    );
  }
  ...
  if (params.knowledge.voiceGuidance?.preferredOpeners?.length) {
    guidanceLines.push(
      `Good opener styles for this persona: ${params.knowledge.voiceGuidance.preferredOpeners.join("; ")}`,
    );
  }

  if (params.knowledge.voiceGuidance?.preferredTransitions?.length) {
    guidanceLines.push(
      `Good transition styles for this persona: ${params.knowledge.voiceGuidance.preferredTransitions.join("; ")}`,
    );
  }

  if (params.knowledge.voiceGuidance?.preferredClosers?.length) {
    guidanceLines.push(
      `Good closing styles for this persona: ${params.knowledge.voiceGuidance.preferredClosers.join("; ")}`,
    );
  }

  if (params.knowledge.voiceGuidance?.avoidPhrases?.length) {
    guidanceLines.push(
      `Avoid these flat or generic phrases: ${params.knowledge.voiceGuidance.avoidPhrases.join("; ")}`,
    );
  }
...
function buildStructuredQuoteReply(params: {
  ...
  const bodyLines = [
    salutation,
    "",
    buildStructuredQuoteOpening(
      "project",
      quoteFacts,
      params.knowledge,
      `${params.latestSubject}:${params.latestEmailText}`,
    ),
    "",
...
function extractQuoteFacts(haystack: string): QuoteFacts {
  ...
  const areaMatch = normalized.match(
    /(\d+(?:\.\d+)?)\s*(?:square metres|square meters|sq m|sqm|m2|m²)/i,
  );
...
function buildTailoredClarifyingQuestions(
  ...
  return questions.filter((question) => {
    ...
    if (
      (quoteFacts.hasSlope ||
        quoteFacts.hasPoorAccess ||
        quoteFacts.hasWaterlogging ||
        quoteFacts.hasClay) &&
...
function buildQuotePreferenceLead(quoteFacts: QuoteFacts): string {
  ...
  if (quoteFacts.finishLevel === "premium") {
    ...
    parts.push(
      " Knowing that you are budget-conscious is helpful because it narrows the finish options.",
    );
  }
...
function sanitiseGeneratedReplyText(text: string): string {
  ...
  cleaned = cleaned.replace(/\b(?:supplier|suppliers)\b/gi, "finish options");
...
function buildAgentFallbackReply(params: {
  ...
  if (params.replyMode === "quote_follow_up_detail") {
    const preferredQuestions =
      params.knowledge?.followUpGuidance?.preferredQuestions?.slice(0, 2) ?? [];
    const lines = [
...
function isSafeOverviewQuestion(params: {
  ...
  const startsLikeOverviewQuestion = /^(tell|what|how|who|can|could|would|please)\b/i.test(
...
function shouldEscalateToHuman(params: {
  ...
  if (
    isPricingQuestion(params.latestEmailText, params.latestSubject) &&
    params.knowledge.pricing.status !== "known"
  ) {
    return {
      action: "escalate",
  ...
  if (
    params.classification.category === "billing_admin" ||
...
export async function generateAgentReply(params: {
  ...
  if (!quoteFollowUpContext) {
    ...
    if (structuredQuoteReply) {
      console.log(
        "Mailbox agent used structured quote reply",
        JSON.stringify({
          subject: params.latestSubject,
  ...
  console.log(
    "Mailbox agent generateReply input",
    JSON.stringify({
      subject: params.latestSubject,
      classification: params.classification.category,
  ...
  if (!generatedText) {
    console.warn(
      "Mailbox agent generateReply returned empty text",
      JSON.stringify({
        subject: params.latestSubject,
        classification: params.classification.category,
        latestEmailPreview: summariseForLog(params.latestEmailText),
...
async function sendHumanEscalationEmail(params: {
  ...
  if (targetMailboxExists) {
    const targetStub = getMailboxStub(
      params.env,
      params.profile.humanEscalationEmail.toLowerCase(),
    );
...
You are ${profile.name}, the ${profile.role} at ${profile.emailAddress}. Use ${profile.tone}.`,
  ...
  async onRequest(request: Request): Promise<Response> {
    ...
    if (url.pathname === "/onNewEmail" && request.method === "POST") {
      ...
      try {
        ...
        console.error(
          "Mailbox agent onNewEmail failed",
          JSON.stringify({
  ...
  async handleNewEmail(emailData: NewEmailPayload) {
    ...
    const { profile, company, knowledge } = mailboxContext;
    console.log(
      "Mailbox agent onNewEmail received",
      JSON.stringify({
    ...
    if (bodyLooksUnsafe) {
      ...
      console.warn(
        "Mailbox agent blocked email due to prompt injection",
        JSON.stringify({
          mailboxId: emailData.mailboxId,
          emailId: emailData.emailId,
          sender: emailData.sender,
      ...
      console.log(
        "Mailbox agent escalated to human",
        JSON.stringify({
          mailboxId: emailData.mailboxId,
          emailId: emailData.emailId,
          threadId: emailData.threadId,
    ...
    console.log(
      "Mailbox agent extracted inbound text",
      JSON.stringify({
        mailboxId: emailData.mailboxId,
        emailId: emailData.emailId,
        sender: emailData.sender,
    ...
    console.log(
      "Mailbox agent classified email",
      JSON.stringify({
        mailboxId: emailData.mailboxId,
        emailId: emailData.emailId,
        sender: emailData.sender,
    ...
    if (classification.category === "spam_irrelevant") {
      console.warn(
        "Mailbox agent ignored email as spam/irrelevant",
        JSON.stringify({
          mailboxId: emailData.mailboxId,
          emailId: emailData.emailId,
          sender: emailData.sender,
    ...
    const threadContext = buildThreadContext(
      fullThread.filter((email): email is EmailFull => Boolean(email)),
    );
    if (threadContext !== "No earlier messages in this thread.") {
      ...
      if (threadLooksUnsafe) {
        ...
        console.warn(
          "Mailbox agent blocked thread due to prompt injection",
          JSON.stringify({
            mailboxId: emailData.mailboxId,
            emailId: emailData.emailId,
            sender: emailData.sender,
    ...
    if (!reply.text) {
      if (profile.escalationMode === "always_reply") {
        ...
        console.warn(
          "Mailbox agent used fallback reply",
          JSON.stringify({
            mailboxId: emailData.mailboxId,
            emailId: emailData.emailId,
            threadId: emailData.threadId,
        ...
        if (decision.action === "draft") {
          ...
          console.log(
            "Mailbox agent drafted fallback reply",
            JSON.stringify({
              mailboxId: emailData.mailboxId,
              emailId: emailData.emailId,
              threadId: emailData.threadId,
        ...
        console.log(
          "Mailbox agent sent fallback reply",
          JSON.stringify({
            mailboxId: emailData.mailboxId,
            emailId: emailData.emailId,
            threadId: emailData.threadId,
