...
const DevInboundEmailBody = z.object({
...
async function triggerAgentOnNewEmail(
  env: Env,
  payload: {
    mailboxId: string
    emailId: string
  ...
  const agentStub = env.EMAIL_AGENT.get(env.EMAIL_AGENT.idFromName(payload.mailboxId))
  try {
    const result = await (agentStub as any).handleNewEmail(payload)
    return {
      ok: true,
      status: 200,
      statusText: 'OK',
...
app.post('/api/v1/dev/inbound-email', async (c) => {
  if (!import.meta.env.DEV) {
    return c.json({ error: 'Not found' }, 404)
  }

  const body = DevInboundEmailBody.parse(await c.req.json())
  const mailboxId = body.mailboxId.toLowerCase()
  ...
  const inReplyTo = body.inReplyTo ? extractMsgId(body.inReplyTo) : null
  const emailReferences = (body.references ?? []).map(extractMsgId)
  let threadId = body.threadId || emailReferences[0] || inReplyTo || messageId

  if (!body.threadId && !inReplyTo && emailReferences.length === 0) {
  ...
  await (stub as any).createEmail(
    Folders.INBOX,
    {
      id: messageId,
      subject: body.subject || '',
      sender: body.from.toLowerCase(),
      recipient: mailboxId,
      cc: null,
      bcc: null,
  ...
  console.log('Triggering local dev onNewEmail', JSON.stringify(payload))
  const agentResult = await triggerAgentOnNewEmail(c.env, payload)

  if (!agentResult.ok) {
    console.error('Local dev onNewEmail returned error', JSON.stringify({
      ...payload,
      status: agentResult.status,
      statusText: agentResult.statusText,
      body: agentResult.body,
    }))
  } else {
    console.log('Local dev onNewEmail completed', JSON.stringify({
      ...payload,
      status: agentResult.status,
      body: agentResult.body,
    }))
...
async function receiveEmail(
  ...
  ctx.waitUntil(
    (async () => {
      ...
      console.log('Triggering mailbox agent onNewEmail', JSON.stringify(payload))
      ...
      try {
        ...
        if (!response.ok) {
          console.error('Mailbox agent onNewEmail returned error', JSON.stringify({
            mailboxId,
            emailId: messageId,
            threadId,
        ...
        console.log('Mailbox agent onNewEmail completed', JSON.stringify({
          mailboxId,
          emailId: messageId,
          threadId,
          status: response.status,
