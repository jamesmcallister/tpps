...
app.post("/api/v1/dev/inbound-email", async (c) => {
  ...
  if (!agentResult.ok) {
    console.error(
      "Local dev onNewEmail returned error",
      JSON.stringify({
        ...payload,
    ...
    console.log(
      "Local dev onNewEmail completed",
      JSON.stringify({
        ...payload,
        status: agentResult.status,
        body: agentResult.body,
...
function sanitizeFilename(filename: string): string {
  return Array.from(filename.replace(/[/\\:*?"<>|]/g, "_"), (char) =>
...
async function receiveEmail(
  ...
  if (parsedEmail.attachments) {
    for (const att of parsedEmail.attachments) {
      ...
      const filename = sanitizeFilename(att.filename || "untitled");
  ...
  ctx.waitUntil(
    (async () => {
      ...
      try {
        ...
        if (!response.ok) {
          console.error(
            "Mailbox agent onNewEmail returned error",
            JSON.stringify({
              mailboxId,
              emailId: messageId,
              threadId,
        ...
        console.log(
          "Mailbox agent onNewEmail completed",
          JSON.stringify({
            mailboxId,
            emailId: messageId,
            threadId,
