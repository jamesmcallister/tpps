...
export default function HomeRoute() {
  ...
  useEffect(() => {
    ...
    const desiredMailboxes = Array.from(new Set([...emailAddresses, ...configuredMailboxEmails]));
    if (desiredMailboxes.length === 0) return;
    const existingEmails = new Set(mailboxes.map((m) => m.email.toLowerCase()));
    const toCreate = desiredMailboxes.filter((addr) => !existingEmails.has(addr.toLowerCase()));
