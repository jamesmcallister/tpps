...
- [ ] **Domain Registration/Transfer:** Move or add `tppslandscapes.com` (or your intended domain) to Cloudflare so it manages your DNS.
- [ ] **Email Routing:** Under **Email** -> **Email Routing**, enable Email Routing for the domain.
- [ ] **Inbound Routing:** Once the Worker is deployed, create a custom address or catch-all route that sends inbound emails (e.g., `hello@tppslandscapes.com`) directly to the `tpps-inbox` Worker.
...
- [ ] **Create an Access Application:** In the Zero Trust dashboard, create an application for the dashboard route (e.g., `inbox.tppslandscapes.com`).
...
- [ ] `DOMAINS` is set to your actual domain (e.g., `tppslandscapes.com`).
